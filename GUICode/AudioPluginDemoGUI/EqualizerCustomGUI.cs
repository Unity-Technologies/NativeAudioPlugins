...
{
    public void DrawSpectrum(Rect r, bool useLogScale, float[] data, float dB_range, float samplerate, float col_r, float col_g, float col_b, float col_a)
    {
    ...
    public bool DrawControl(IAudioEffectPlugin plugin, Rect r, float samplerate)
    {
        ...
        if (Event.current.type == EventType.Repaint)
        {
            ...
            if (showSpectrum)
            {
                ...
                DrawSpectrum(r, useLogScale, spec, dbRange, samplerate, 0.3f, 1.0f, 0.3f, 0.5f * blend);

                plugin.GetFloatBuffer("OutputSpec", out spec, specLen);
                DrawSpectrum(r, useLogScale, spec, dbRange, samplerate, 1.0f, 0.3f, 0.3f, 0.5f * blend);
            }
